      ...
      if (decision.action === 'auto_reply') {
        ...
        if (channel === 'Email') {
          try {
            ...
            const info = await emailOrchestrator.sendGuestReply({
              hotelId,
              conversationId: conversation.id,
              to: guest.email || senderIdentity,
              subject,
              html: `<p>${decision.response.replace(/\n/g, '<br>')}</p>`,
              references: emailMessageId,
              inReplyTo: emailMessageId
            });
            outEmailMsgId = info?.messageId || info?.providerMessageId || null;
          } catch (err) {
            console.error('Failed to dispatch Email message:', err.message);
            await conversationService.logActivity(conversation.id, 'Email Dispatch Failed', err.message);
          }
